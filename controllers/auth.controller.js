...
exports.signup = async (req, res) => {
  try {
    ...
    Mail(
      email,
      "You have successfully created account at Ovigo. Now you can list your properties."
    )
  ...
  try {
    ...
    if (existingUser) {
      return res.status(409).send({
        message: "This email already is being used!",
      })
    }
    ...
    if (email) {
      ...
      const verification_link = `${
        host.includes("localhost") ? "http" : "https"
      }://${host}/signup?email=${email}&token=${token}`
